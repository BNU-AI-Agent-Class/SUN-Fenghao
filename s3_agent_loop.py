...
# s3：Agent 循环。
# 目标：模型可以请求程序执行 shell 命令，然后观察执行结果。
...
system_prompt = """你必须用以下两种格式之一回复：
- 需要执行 shell 命令时：命令:XXX
- 任务完成时：完成:XXX

规则：
- 命令: 后面的 XXX 必须是一条纯 shell 命令，不要解释。
- 完成: 后面的 XXX 是给用户看的简短总结。
- 如果用户要求你读取、查看、分析某个本地文件，必须先回复“命令:XXX”读取文件。
- 没有看到系统反馈的文件内容前，禁止声称自己已经读过文件。"""
